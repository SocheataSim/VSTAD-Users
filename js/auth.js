...
const AUTH = {
    ...
    // Login with email and password - FIXED
    async login(email, password) {
        try {
    ...
    // Login with Google (Firebase) - FIXED
    async loginWithGoogle(idToken, userInfo) {
        try {
            console.log('Google login with token:', idToken);
            console.log('User info:', userInfo);
            ...
            if (response.ok) {
                ...
                // If login fails, store Firebase data temporarily
                this.storeAuthData({
                    access_token: idToken,
                    token_type: 'Bearer',
                    user_data: {
    ...
    // Store authentication data in localStorage - IMPROVED
    storeAuthData(data) {
