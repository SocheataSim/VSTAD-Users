...
// Helper function to update comments UI
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.location.pathname.includes('display')) {
    ...
    if (videoId) {
      ...
      // Start auto-refresh (every 10 seconds to catch changes quickly)
...
// // Function to load video interactions
// async function loadVideoInteractions(videoId) {
//   try {
//     const response = await fetch(
//       `https://vstad-api.cheatdev.online/api/interactions/videos?videoId=${videoId}`
...
//     // Update UI with interaction data (customize based on your needs)
//     if (data) {
//       updateInteractionsUI(data);
//     }
...
// // Function to load comments (you'll need to implement this based on your API)
// async function loadComments(videoId) {
//   try {
//     const response = await fetch(
//       `https://vstad-api.cheatdev.online/api/comments?videoId=${videoId}`
//     );
...
//     // Update UI with comments (customize based on your needs)
//     if (data) {
...
// // Function to check user interactions (you'll need to implement this based on your API)
// async function checkUserInteractions(videoId) {
//   try {
//     const response = await fetch(
//       `https://vstad-api.cheatdev.online/api/interactions/user?videoId=${videoId}`
//     );
...
//     // Update UI to reflect user's interactions (customize based on your needs)
//     if (data) {
//       updateUserInteractionsUI(data);
//     }
...
// // Helper function to update interactions UI
// function updateInteractionsUI(data) {
...
// // Helper function to update user interactions UI
// function updateUserInteractionsUI(data) {
//   // Customize this based on your HTML structure
...
//       // const refreshInterval = startAutoRefresh(videoId, 30);
      
//       // Optional: Stop refresh when user leaves the page
//       // window.addEventListener('beforeunload', () => {
//       //   if (refreshInterval) {
//       //     clearInterval(refreshInterval);
//       //   }
...
// // Function to load all video interactions
// async function loadVideoInteractions(videoId) {
//   try {
//     const [likes, comments, shares] = await Promise.all([
//       loadVideoLikes(videoId),
...
//     updateCommentsUI(comments);
//     updateSharesCount(shares.length);
...
//   if (commentsListElement && Array.isArray(comments)) {
//     if (comments.length === 0) {
//       commentsListElement.innerHTML = '<p class="text-gray-500 dark:text-gray-400 text-center py-4">No comments yet. Be the first to comment!</p>';
//       return;
//     }
...
//       return `
//         <div class="flex space-x-3 py-3 border-b border-gray-200 dark:border-gray-700 last:border-0">
//           <img src="${profileImage}" 
//                alt="${comment.user.username}" 
//                class="w-10 h-10 rounded-full object-cover">
//           <div class="flex-1">
//             <div class="flex items-center gap-2 mb-1">
//               <span class="font-semibold text-gray-900 dark:text-white">${comment.user.full_name || comment.user.username}</span>
